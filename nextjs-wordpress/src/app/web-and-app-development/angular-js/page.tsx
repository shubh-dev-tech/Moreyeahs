...
async function getPageData(): Promise<PageData> {
  ...
  const slugVariations = [
    'web-and-app-development/angular-js',
    'angular-js',
    'angular',
  ...
  for (const slug of slugVariations) {
    try {
      ...
      if (page) {
        console.log(`[Angular.js Page] Found page with slug: ${slug}`);
        return page;
      }
    } catch (error) {
      console.warn(`[Angular.js Page] Failed to fetch with slug ${slug}:`, error);
    }
  }
  ...
  for (const slug of slugVariations) {
    try {
      ...
      if (page) {
        console.log(`[Angular.js Page] Found page via fallback with slug: ${slug}`);
        return {
          id: page.id,
          title: page.title?.rendered || 'Angular.js Development Services',
      ...
      console.warn(`[Angular.js Page] Fallback failed for slug ${slug}:`, error);
    }
  }
  
  // If no page found, return default content
  console.warn('[Angular.js Page] No WordPress page found, using default content');
  return {
    id: 0,
    title: 'Angular.js Development Services',
    content: '<p>Angular.js development services coming soon...</p>',
    slug: 'angular-js',
    blocks: []
  };
}
...
export async function generateMetadata() {
  return generatePageMetadata('web-and-app-development/angular-js');
}

export default async function AngularJsPage() {
